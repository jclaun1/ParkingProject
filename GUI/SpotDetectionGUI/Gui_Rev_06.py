...
def printcoords(event):
    ...
    global count, xVals, yVals, spotLabels, currentLabel, cropped_img, innerSpots
...
def crop_inner():
    ...
    rightCanvas.bind("<Button 1>", getInnerSpots)
    return
...
def getInnerSpots(event):

    global count, xVals, yVals, spotLabels, currentLabel, cropped_img, innerSpots, innerIndex, innerCount
    ...
    x1 = rightCanvas.canvasx(event.x)
    y1 = rightCanvas.canvasy(event.y) 
    rightCanvas.create_oval(x1-2, y1-2, x1+2, y1+2, fill='blue')

    innerSpots[innerIndex+1].append(x1)
    innerSpots[innerIndex+1].append(y1)

    if innerCount == 4:
        #innerSpots = [ [spot1], [spot2], ... [x1, y1, x2, y2, x3, y3, x4, y4] ]
        innerCount = 0
        innerIndex += 1
        print(innerSpots)
...
edit_menu.add_command(label = "Crop Inner", command=crop_inner)
edit_menu.add_separator()
